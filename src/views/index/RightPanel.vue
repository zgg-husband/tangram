<template>
  <div class="right-board">
    <el-tabs v-model="currentTab" class="center-tabs">
      ...
      <el-tab-pane label="大纲树" name="nodeTree" />
    </el-tabs>
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
          ...
          <el-form-item v-if="activeData.__config__.changeTag" label="组件类型">
            ...
            >
              <el-option-group v-for="group in tagList" :key="group.label" :label="group.label">
                ...
                >
                  <svg-icon
                    v-if="item.__config__.tagIcon.type === 'el'"
                    class="node-icon"
                    :icon-class="item.__config__.tagIcon.logoName"
                  />
                  <!-- eslint-disable-next-line -->
                  <i
                    v-if="item.__config__.tagIcon.type === 'fi'"
                    class="node-icon iconfont"
                    :class="item.__config__.tagIcon.logoName"
                  >
                  </i>
        ...
        <el-form v-show="currentTab === 'nodeTree'" size="small" label-width="90px">
          ...
          <el-tree
            ...
            :data="treeData"
            node-key="renderKey"
            :current-node-key="activeData.__config__.renderKey"
            default-expand-all
            highlight-current
            draggable
            render-after-expand
          ...
          >
            <span slot-scope="{node,data}">
              <span class="node-label">
                <svg-icon
                  v-if="data.tagIcon.type==='el'"
                  class="node-icon"
                  :icon-class="data.tagIcon.logoName"
                />
                <!-- eslint-disable-next-line -->
                  <i
                  v-if="data.tagIcon.type==='fi'"
                  class="iconfont"
                  :class="data.tagIcon.logoName"
                >
                </i>
                {{ node.label }}
              </span>
...
import { saveFormConf, saveActiveData } from '@/utils/db'
...
export default {
  ...
  data() {
    return {
      // activeDatarenderKey: drawingDefalut[0].__config__.renderKey,
      currentTab: 'field',
      currentNode: null,
      dialogVisible: false,
      iconsVisible: false,
  ...
  computed: {
    treeData() {
      ...
      const data = this.recursive(drawingList)
      return data
    },
    documentLink() {
  ...
  watch: {
    ...
    'activeData.__config__.span': {
      handler(val) {
        saveActiveData(this.activeData)
      }
    }
  },
  mounted() {
  ...
  methods: {
    nodeDragEnd(...args) {
      this.$emit('structurechangeupdatedrawinglist', args)
    },
    nodeDragEnter(...args) {
      // console.log(args, 'nodeDragEnter')
      // debugger
    },
    nodeDragLeave(...args) {
      // console.log(args, 'nodeDragLeave')
    },
    currentChange(data, el) {
      this.$emit('active-id-change', data.__data__)
    },
    recursive(data) {
      // 处理大纲树数据
      return data.map(item => ({
        label: item.__config__.label || item.__config__.componentName,
        children: item.__config__.children ? this.recursive(item.__config__.children) : undefined,
    ...
    spanChange(val) {
      // this.activeData.__config__.span = val
    ...
    tagChange(tagIcon) {
      ...
      // this.$refs['my-tree'].setCurrentKey(this.activeData.__config__.renderKey)
